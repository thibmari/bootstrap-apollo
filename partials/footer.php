<div class="container">
    <div class="row">

        <div class="col-md-4">

        </div>

        <div class="col-md-4">
            <img src="images/logo.png" alt="logo footer" />
            <h2>Contacteer ons</h2>

            <ul>
                <li>IMMO APOLLO</li>
                <li>Albert I Laan 153/c</li>
                <li>8620 Nieuwpoort</li>
            </ul>
            <ul>
                <li><a href="tel:003258241414">+32 58 24 14 14</a></li>
                <li><a href="mailto:info@immo-apollo.be">info@immo-apollo.be</a></li>
            </ul>
            <ul>
                <li>Vastgoedmakelaar-bemiddelaar 201807</li>
                <li><a target="_blank" href="http://www.biv.be/sites/default/files/download_document/reglement-van-plichtenleer-deontologie-vastgoedmakelaars.pdf">Deontologische code van het BIV</a></li>
                <li>BE 0473.821.145</li>
            </ul>
            <p class="padding-left-cs">A <a target="_blank" href="http://www.ibault.com">iBAULT</a> WEBSITE</p>
        </div>

    </div>
    <div class="text-lg-center sitemap">
       <ul>
            <li><a href="index.php">Home</a></li>
            <li><a href="over">Over ons</a></li>
            <li><a href="nieuwbouw">Nieuwbouw</a></li>
            <li><a href="kopen">Te koop</a></li>
            <li><a href="spanje">Zuid Spanje</a></li>
            <li><a href="schatting">Gratis schatting</a></li>
            <li><a href="kantoor">Kantoor</a></li>
            <li><a href="contact">Contact</a></li>
        </ul>
    </div>
</div>